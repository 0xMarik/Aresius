...
// PITCHFORK / zipped: walk all parameter value-lists in lockstep,
// stop at the shortest one, one target per index.
fn build_zipped_fuzz_requests(session: &FuzzerSession) -> Vec<FuzzTarget> {
    ...
    // Doesn't depend on `i`, so sort once outside the loop.
...
async fn process_zipped_fuzzer_session(
    app: AppHandle,
    ...
    let aggregator = tokio::spawn(async move {
        ...
        tracing::debug!("Starting zipped fuzz");
        loop {
            tokio::select! {
                _ = ticker.tick() => {
                    if !buffer.is_empty() {
