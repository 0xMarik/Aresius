...
import { httpStreamLanguage } from './http-parser.component';
...
const FuzzRequestPayload = () => {
  ...
  const syncPlaceholders = () => { // declared in useEffect with rawRequest as param
    // const detectedPlaceholders = extractPlaceholders(rawRequest || '');
    // const existingPlaceholders = parameters.map(p => p.name);

    // console.log({ detectedPlaceholders })
    // Add new placeholders
    // detectedPlaceholders.forEach(placeholder => {
    //   if (!existingPlaceholders.includes(placeholder)) {
    //     const duplicateCount = parameters.filter(p => p.name === placeholder).length;
    //     const parameterName = duplicateCount > 0 ? `${placeholder}_${duplicateCount + 1}` : placeholder;
    ...
    if (editorRef.current && !viewRef.current) {
      ...
      const state = EditorState.create({
        ...
        extensions: [
          ...
          keymap.of([indentWithTab]) // Add this line
        ]
      });
  ...
  // Auto-sync placeholders when raw request changes
  useEffect(() => {
    if (rawRequest) {
      syncPlaceholders();
      // checkForDuplicates();
